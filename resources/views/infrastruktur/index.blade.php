...
<div class="row g-3">
    ...
    <div class="col-lg-12"><div class="card"><div class="card-header fw-semibold">Distribusi Tahun Renewal Sewa Lahan</div><div class="card-body"><div id="infra-renewal-chart" style="height:300px"></div></div></div></div>
...
$(function () {
    ...
    $.getJSON(@json(route('infrastruktur.dashboard.data')), function (data) {
        ...
        Highcharts.chart('infra-status-chart', themed({ chart: { type: 'pie' }, plotOptions: { pie: { dataLabels: { enabled: true, format: '{point.name}: {point.y} ({point.percentage:.1f}%)' } }, series: clickOptions }, series: [{ name: 'Site', data: drilldown(data.status) }] }));
        Highcharts.chart('infra-owner-chart', themed({ chart: { type: 'pie' }, plotOptions: { pie: { innerSize: '55%', dataLabels: { enabled: true, format: '{point.name}: {point.y} ({point.percentage:.1f}%)' } }, series: clickOptions }, series: [{ name: 'Site', data: drilldown(data.owners) }] }));
        Highcharts.chart('infra-source-chart', themed({ chart: { type: 'pie' }, plotOptions: { pie: { innerSize: '55%', dataLabels: { enabled: true, format: '{point.name}: {point.y} ({point.percentage:.1f}%)' } }, series: clickOptions }, series: [{ name: 'Site', data: drilldown(data.sources) }] }));
        Highcharts.chart('infra-renewal-chart', themed({ chart: { type: 'column' }, xAxis: { type: 'category' }, yAxis: { title: { text: 'Site unik' } }, plotOptions: { series: sewaRenewalClickOptions }, series: [{ name: 'Site', data: drilldown(data.renewal_years) }] }));
        Highcharts.chart('infra-contract-chart', themed({ chart: { type: 'column' }, xAxis: { type: 'category' }, yAxis: { title: { text: 'Nilai kontrak' } }, tooltip: { pointFormat: '<b>Rp {point.y:,.0f}</b>' }, plotOptions: { series: clickOptions }, series: [{ name: 'Nilai', data: drilldown(data.contract_by_source) }] }));
    }).fail(function (xhr) {
